...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    for gold in golds:
        ...
        current_acc_reward = 0.2 * em_reward + 0.03 * bleu4_reward + 0.03 * rouge_l_reward + 0.2 * cider_n_reward
    ...
    env_tool = extra_info.get("tool_rewards")
    env_tool = [float(x) for x in env_tool if isinstance(x, (int, float))]
    if isinstance(env_tool, (list, tuple)) and env_tool:
        base_tool = max(env_tool)
    else:
        base_tool = 1.0 if has_tool_usage else 0.0  # fallback credit

    # SOFT gate instead of hard threshold
    tool_reward = base_tool * min(1.0, max(0.0, acc_reward))  # scales with accuracy but never hard-zeros
